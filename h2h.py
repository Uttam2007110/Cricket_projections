...
comp = 'lpl'; year = '23'; unique_combos = 5
...
#home = ["Barbados Royals"]; opps = ["Trinbago Knight Riders"]; venue = [""]
#select custom date
#custom = dt.datetime(2023,8,4) #year,month,date
...
def gw_projection(a,b,input_file1,input_file,factor,v):
    ...
    print(t1,round(s1*c2*league_avg*(bat_game.loc[bat_game['team']==t1,'usage'].sum()),2),"runs in",round(20*factor*bat_game.loc[bat_game['team']==t1,'usage'].sum(),2),"overs")
    print(t2,round(s2*c1*league_avg*(bat_game.loc[bat_game['team']==t2,'usage'].sum()),2),"runs in",round(20*factor*bat_game.loc[bat_game['team']==t2,'usage'].sum(),2),"overs")
    print(" ")
...
def final_projections(bat_game,bowl_game):
    ...
    for x in names:
        ...
        bowl_game.drop(bowl_game[bowl_game['usage'] < 0.05].index, inplace = True)
    ...
    elif(ex22 == 1): final['total'] = final['bat'] + final['bowl']
...
elif(coversoff == 0 and ex22 == 0 and writer == 1):
